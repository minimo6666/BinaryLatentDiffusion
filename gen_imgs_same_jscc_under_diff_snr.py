...
def main(H):
    ...
    # 计算实际需要的 batch 数，让 tqdm 进度条显示正确的分母
    num_batches_needed = (num_eval_images + H.batch_size - 1) // H.batch_size

    # 5. 开始推理计算
    with torch.no_grad():
        for data in tqdm(val_loader, desc=f"Processing Test SNR {H.snr}",
                         total=num_batches_needed, disable=misc.get_rank() != 0):
    ...
    if misc.get_rank() == 0:
        ...
        # 去重写入：读取已有条目，去掉当前 SNR 的旧值，插入新值，按 SNR 排序重写
        existing_lines = []
        if os.path.exists(results_txt_path):
            with open(results_txt_path, 'r') as f:
                for line in f:
                    line = line.strip()
                    if not line or line.startswith('===') or line.startswith('Test_SNR') or line.startswith('---'):
                        continue
                    parts = line.split(',')
                    if len(parts) >= 2:
                        try:
                            old_snr = float(parts[0].strip())
                            if abs(old_snr - H.snr) > 0.01:  # 不是当前 SNR 的保留
                                existing_lines.append((old_snr, line))
                        except ValueError:
                            continue

        # 插入当前 SNR 的新结果
        existing_lines.append((H.snr, f"{H.snr:>12}, {avg_psnr:>16.4f}"))
        # 按 SNR 排序
        existing_lines.sort(key=lambda x: x[0])
